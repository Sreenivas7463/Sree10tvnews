...
newsRouter.get('/:p', async(req, res) => {
    ...
    try {
        const newsAPI = await axios.get(`https://diey8xpfs90ha.cloudfront.net/wp-json/wp/v2/posts?_embed&per_page=100&page=${pageNo}`)
        res.render('news', { articles : newsAPI.data})
    } catch (err) {
        if(err.response) {
            res.render('news', { articles : null })
...
newsRouter.get('/:id/:mid', async(req, res) => {
    ...
    try {
        const newsAPI = await axios.get(`https://diey8xpfs90ha.cloudfront.net/wp-json/wp/v2/posts/${articleID}`)
        const newsMedia = await axios.get(`https://diey8xpfs90ha.cloudfront.net/wp-json/wp/v2/media/${mediaID}`) //added image by Sreenivas
...
newsRouter.post('', async(req, res) => {
    ...
    try {
        const newsAPI = await axios.get(`https://diey8xpfs90ha.cloudfront.net/wp-json/wp/v2/posts?search=${search}`)
